...
- **Best-effort** : la résolution (téléchargement, re-recherche, génération) a un budget de temps et de concurrence dédié (`MEDIA_RESOLVE_TIMEOUT_SECONDS`, `MEDIA_RESOLVE_CONCURRENCY`) ; un bloc `image` dont la résolution échoue est simplement retiré du cours, jamais laissé sans image. Pour cette raison, un bloc `image` ne compte jamais comme le visuel de la règle « visuel d'abord » (`app/services/visual_validation.py`).
